...
class LabelFrequencyBasedVotingClassifier(BaseEstimator, ClassifierMixin):
    '''
        Trains 2 separate classifiers such that:
        1st classifier focuses on most frequent labels/tags
        and 2nd classifier focuses on less frequent labels/tags.
    '''

    def __init__(self, clf1, clf2, weights=None):
        assert weights is None or sum(weights) == 1.0
        self.clf1 = clf1
        self.clf2 = clf2
        self.label_assignments = {}
        self.upper_lower_threshold = 0
    ...
    def fit(self, X, y):
        self.label_assignments = {}
        for y_i in y:
            for label_idx, label_value in enumerate(y_i):
                if label_idx not in self.label_assignments:
                    self.label_assignments[label_idx] = 0
                self.label_assignments[label_idx] += label_value

        sorted_labels_by_value = sorted(self.label_assignments.items(), key=operator.itemgetter(1))
        self.upper_lower_threshold = sorted_labels_by_value[int(float(len(sorted_labels_by_value))/1.2)][1]

        X_clf1, y_clf1 = [], []
        X_clf2, y_clf2 = [], []

        for index, y_i in enumerate(y):
            n_label_assignments = 0
            average_label_frequency = 0
            for label_idx, label_value in enumerate(y_i):
                if label_value == 1:
                    n_label_assignments += 1
                    average_label_frequency += self.label_assignments[label_idx]

            average_label_frequency = float(average_label_frequency)/float(n_label_assignments)

            if average_label_frequency < self.upper_lower_threshold:
                X_clf1.append(X[index].toarray()[0])
                y_clf1.append(y[index].tolist())
            else:
                X_clf2.append(X[index].toarray()[0])
                y_clf2.append(y[index].tolist())

        _logger.info('X_clf1 = %d', len(X_clf1))
        _logger.info('X_clf2 = %d', len(X_clf2))
        self.clf1[1].fit(sparse.csr_matrix(X_clf1), np.array(y_clf1)) # fit 1st classifier
        self.clf2[1].fit(sparse.csr_matrix(X_clf2), np.array(y_clf2)) # fit 2nd classifier


    def predict_proba(self, X):
        all_probabs_clf1 = self.clf1[1].predict_proba(X)
        all_probabs_clf2 = self.clf2[1].predict_proba(X)
        assert len(all_probabs_clf1) == len(all_probabs_clf2)

        final_probabilities = []
        for idx, instance_prob_clf1 in enumerate(all_probabs_clf1):
            temp = []
            for label_idx in range(len(instance_prob_clf1)):
                if self.weights is None:
                    temp += [max(all_probabs_clf1[idx][label_idx], all_probabs_clf2[idx][label_idx])]
                else:
                    temp += [float(all_probabs_clf1[idx][label_idx] * self.weights[0] +
                                   all_probabs_clf2[idx][label_idx] * self.weights[1]) / 2.0]
            final_probabilities.append(temp)
        return np.array(final_probabilities)

    def predict(self, X):
        y_predicted_probab = self.predict_proba(X)
        y_predicted_list = []
        for probabilities in y_predicted_probab:
            y_predicted_list.append(map(lambda probab: float(probab >= 0.5), probabilities))
        return np.array(y_predicted_list)


class CustomVotingClassifier(BaseEstimator, ClassifierMixin):

    def __init__(self, clfs, weights=None):
        assert weights is None or (len(weights) == len(clfs) and sum(weights) == 1.0)
        assert len(clfs) > 1
        self.clfs = clfs
        self.weights = weights
    ...
    def predict_proba(self, X):
        ...
        assert len(clf_probabilities) == len(self.clfs)
        assert len(clf_probabilities) > 0
        assert len(clf_probabilities[0]) > 0, 'No instances?! -> This should never happen'
        assert len(clf_probabilities[0][0]) > 0, 'No labels?! -> This should never happen'

        n_instances = len(clf_probabilities[0])
        n_labels = len(clf_probabilities[0][0])

        all_instances_probabilities = []
        for instance_idx in range(n_instances):
            instance_probabilities = []
            for label_idx in range(n_labels):
                probab_sum = 0.0
                for clf_idx in range(len(self.clfs)):
                    weight = self.weights[clf_idx] if self.weights != None else 1.0
                    probab_sum += clf_probabilities[clf_idx][instance_idx][label_idx] * weight
                probab_sum /= len(self.clfs)
                instance_probabilities += [probab_sum]

            all_instances_probabilities.append(instance_probabilities)
        return np.array(all_instances_probabilities)
...
class StackedGeneralizer(BaseEstimator, ClassifierMixin):
    """Base class for stacked generalization classifier models
       based on: https://github.com/dustinstansbury/stacked_generalization/blob/master/stacked_generalizer.py
    """
