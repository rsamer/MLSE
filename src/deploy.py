...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        helper.error("usage: python deploy.py <hostname_or_ip> <port> <username> <deploy>")
    ...
    ssh_client = None
    ssh_client = createSSHClient(sys.argv[1], int(sys.argv[2]), sys.argv[3], password)
    if ssh_client is None:
        helper.error("Deployment failed!")

    if sys.argv[4].lower() != "--not-deploy":
    ...
    _, stdout, stderr = ssh_client.exec_command('cd MLSE/src/ && screen -dmS MLSE.%s python -m main ../data/%s' % data_set)
    for line in stdout:
